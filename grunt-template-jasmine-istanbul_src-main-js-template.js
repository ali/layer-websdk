...
exports.process = function (grunt, task, context) {
	...
	task.phantomjs.on('jasmine.coverage', function (coverage) {
	    /* Support for ignoreFiles */
	    var ignoreFiles = context.options.ignoreFiles;
	    var files = Object.keys(coverage);
	    ignoreFiles.forEach(function(ignoreFile) {
    		files.forEach(function(file) {
		    if (file.indexOf(ignoreFile) != -1) delete coverage[file];
		});
	    });
		...
		if (context.options.thresholds) {
			checkThresholds(collector, context.options);
		}
	});
